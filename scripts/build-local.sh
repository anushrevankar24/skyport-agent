...
if [ -f "scripts/build-config-local.env" ]; then
    echo "Loading build configuration from build-config-local.env"
    source scripts/build-config-local.env
else
    echo "Warning: build-config-local.env not found, using defaults"
...
if [ $? -eq 0 ]; then
    ...
    echo "   Tunnel Domain: $SKYPORT_TUNNEL_DOMAIN"
    echo ""
    echo "Users can now run: ./skyport-local login"
